...
create trigger set_whatsapp_settings_updated_at before update on public.whatsapp_settings for each row execute procedure public.set_updated_at();

drop trigger if exists set_whatsapp_connections_updated_at on public.whatsapp_connections;
create trigger set_whatsapp_connections_updated_at before update on public.whatsapp_connections for each row execute procedure public.set_updated_at();

drop trigger if exists set_whatsapp_qr_sessions_updated_at on public.whatsapp_qr_sessions;
create trigger set_whatsapp_qr_sessions_updated_at before update on public.whatsapp_qr_sessions for each row execute procedure public.set_updated_at();
